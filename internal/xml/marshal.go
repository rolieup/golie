...
func (p *printer) writeStart(start *StartElement) error {
	...
	if start.Name.Space != "" { // tag starts with <.Space:.Local
		...
		prefixToPrint := ""
		//fmt.Println(start.Name.Space, start.Name.Local)
		//fmt.Println(start.Attr)

		_, err := url.ParseRequestURI(start.Name.Space)
		if err != nil {
			prefixToPrint = start.Name.Space
		}
...
func (p *printer) writeEnd(name Name) error {
	...
	_, err := url.ParseRequestURI(name.Space)
	if err != nil {
		if name.Space != "" {
